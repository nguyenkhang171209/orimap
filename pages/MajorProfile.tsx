...
const MajorProfile: React.FC<MajorProfileProps> = ({ majorId, onNavigate }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 md:py-10 space-y-6 md:space-y-10">
      {/* Header Info */}
      <div className="bg-primary-dark rounded-3xl md:rounded-[3.5rem] p-6 md:p-16 text-white flex flex-col md:flex-row justify-between items-center gap-8 md:gap-12 relative overflow-hidden shadow-2xl shadow-primary-dark/20">
        <div className="relative z-10 space-y-4 md:space-y-6 max-w-2xl text-center md:text-left">
          ...
          <div className="flex items-center gap-2 md:gap-3 text-primary-100 text-base md:text-xl font-bold justify-center md:justify-start">
            <GraduationCap className="w-5 h-5 md:w-6 md:h-6" />
            {major.university}
          </div>
          <p className="text-primary-50/60 text-sm md:text-lg leading-relaxed">{major.description || 'Chương trình đào tạo chuyên sâu giúp sinh viên làm chủ kiến thức và kỹ năng thực tế trong ngành.'}</p>
          <div className="flex flex-wrap gap-3 justify-center md:justify-start pt-2 md:pt-4">
            ...
            <div className="flex items-center gap-2 bg-white/10 px-4 md:px-5 py-2 md:py-3 rounded-xl md:rounded-2xl border border-white/20">
              <Users className="w-4 h-4 md:w-5 md:h-5 text-primary-100" />
        ...
        <div className="relative z-10 grid grid-cols-2 sm:grid-cols-2 gap-4 md:gap-6 w-full md:w-auto">
          <div className="bg-white/10 backdrop-blur-xl p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-white/10 text-center shadow-2xl">
            <p className="text-primary-100 text-[8px] md:text-[10px] font-black uppercase tracking-widest mb-1 md:mb-2 opacity-60">Lương Khởi Điểm</p>
            <p className="text-base md:text-2xl font-black">{major.avgSalary || '15-30 Triệu'}</p>
          </div>
          <div className="bg-white/10 backdrop-blur-xl p-4 md:p-8 rounded-2xl md:rounded-[2.5rem] border border-white/10 text-center shadow-2xl">
            <p className="text-primary-100 text-[8px] md:text-[10px] font-black uppercase tracking-widest mb-1 md:mb-2 opacity-60">Độ Cạnh Tranh</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-10">
        ...
        <div className="lg:col-span-2 space-y-6 md:space-y-10">
          ...
          <section className="bg-white p-6 md:p-14 rounded-3xl md:rounded-[3.5rem] border border-slate-100 shadow-sm space-y-8 md:space-y-10">
            <h3 className="text-xl md:text-3xl font-black flex items-center gap-3 md:gap-4 text-slate-900">
              <ChevronRight className="w-6 h-6 md:w-8 md:h-8 text-primary bg-primary-50 rounded-xl md:rounded-2xl p-1" /> Lộ trình học tập 4 năm
            </h3>
            <div className="space-y-4 md:space-y-6">
              {(major.timeline || [
                ...
                <div key={idx} className="relative flex gap-4 md:gap-8 pb-6 md:pb-10 last:pb-0">
                  ...
                  <div className="flex-shrink-0 w-8 h-8 md:w-12 md:h-12 rounded-lg md:rounded-[1.2rem] bg-primary-50 border-2 md:border-4 border-white shadow-lg flex items-center justify-center font-black text-primary-dark text-sm md:text-lg z-10">
                    {item.year}
                  </div>
            ...
            <button 
              ...
              className="w-full bg-secondary text-white py-4 md:py-6 rounded-xl md:rounded-[2rem] font-black text-base md:text-xl hover:bg-secondary-dark transition-all flex items-center justify-center gap-2 md:gap-3 shadow-xl shadow-secondary/20 active:scale-95"
            >
          ...
          <section className="bg-white p-6 md:p-14 rounded-3xl md:rounded-[3.5rem] border border-slate-100 shadow-sm space-y-6 md:space-y-8">
            <h3 className="text-xl md:text-3xl font-black text-slate-900 flex items-center gap-3 md:gap-4">
              <Briefcase className="w-6 h-6 md:w-8 md:h-8 text-primary bg-primary-50 rounded-xl md:rounded-2xl p-1" /> Vị trí công việc mục tiêu
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              {['Chuyên viên cao cấp', 'Trưởng nhóm kỹ thuật', 'Quản lý dự án', 'Chuyên gia tư vấn'].map(job => (
                <div key={job} className="flex items-center gap-3 md:gap-4 p-4 md:p-6 bg-slate-50 rounded-2xl md:rounded-[2rem] border border-slate-100 group hover:bg-white hover:border-primary-100 transition-all">
                  ...
                  <span className="font-bold text-slate-700 text-sm md:text-lg group-hover:text-primary">{job}</span>
                </div>
              ))}
            </div>
          </section>
        ...
        <div className="space-y-6 md:space-y-10">
              ...
              {(major.skills || [
                ...
                <div key={idx} className="space-y-2 md:space-y-3">
                  ...
                  <div className="w-full h-2.5 md:h-3 bg-slate-100 rounded-full overflow-hidden p-0.5">
                    <div className="h-full bg-gradient-to-r from-primary to-primary-dark rounded-full shadow-lg shadow-primary/30" style={{ width: `${skill.level}%` }}></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <section className={`p-6 md:p-10 rounded-3xl md:rounded-[3rem] border space-y-4 md:space-y-6 ${
            ...
            major.aiRisk === 'medium' ? 'bg-amber-50 border-amber-100' : 'bg-red-50 border-red-100'
          }`}>
            <h3 className={`text-lg md:text-xl font-black uppercase tracking-widest ${
              major.aiRisk === 'low' ? 'text-green-800' :
              major.aiRisk === 'medium' ? 'text-amber-800' : 'text-red-800'
            }`}>Đánh giá AI Risk</h3>
            <p className={`text-base md:text-lg font-medium leading-relaxed ${
              major.aiRisk === 'low' ? 'text-green-700' :
              major.aiRisk === 'medium' ? 'text-amber-700' : 'text-red-700'
            }`}>
