    ...
    return (
      <div className="max-w-6xl mx-auto px-4 py-8 md:py-20 space-y-10 animate-in fade-in duration-700">
        <div className="text-center space-y-4">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-primary-50 text-primary rounded-full text-xs font-black uppercase tracking-widest">
            <Sparkles className="w-4 h-4" /> Career Vector Profile Generated
          </div>
          <h1 className="text-4xl md:text-6xl font-black text-slate-900 tracking-tighter">
            Hồ sơ <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-secondary">Nghề nghiệp AI</span>
          </h1>
        </div>
          ...
          <div className="lg:col-span-5 space-y-8">
            <div className="bg-white p-8 rounded-[2.5rem] border border-slate-200 shadow-xl">
              <h3 className="text-sm font-black text-slate-400 uppercase tracking-widest mb-8 flex items-center gap-2">
                <Target className="w-4 h-4 text-primary" /> Mô hình Holland (RIASEC)
              </h3>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="bg-primary p-6 rounded-[2rem] text-white shadow-lg shadow-primary/20">
              ...
              <div className="bg-secondary p-6 rounded-[2rem] text-white shadow-lg shadow-secondary/20">
          ...
          <div className="lg:col-span-7 space-y-8">
            <div className="bg-white p-8 md:p-10 rounded-[2.5rem] border border-slate-200 shadow-xl space-y-8">
              ...
              <div className="space-y-6">
                <div>
                  <h4 className="text-xs font-black text-primary uppercase tracking-widest mb-3">Điểm mạnh nổi bật</h4>
                  <div className="flex flex-wrap gap-2">
                    {profile.strengths.map((s, i) => (
                      <span key={i} className="px-4 py-2 bg-primary-50 text-primary rounded-xl text-xs font-bold border border-primary-100">
                        {s}
                      </span>
                    ))}
                  </div>
                ...
                {profile.contradictions.length > 0 && (
                  <div className="p-4 bg-amber-50 border border-amber-100 rounded-2xl flex items-start gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-black text-amber-600 uppercase tracking-widest mb-1">Phát hiện mâu thuẫn nội tại</h4>
                      <ul className="text-[11px] text-amber-700 font-medium list-disc pl-4">
              ...
              <div className="pt-6 border-t border-slate-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {MAJORS.slice(0, 4).map((major) => (
                    <div key={major.id} className="p-4 rounded-2xl border border-slate-100 hover:border-primary-200 transition-all group cursor-pointer">
                      <p className="font-black text-slate-800 text-sm group-hover:text-primary transition-colors">{major.majorName}</p>
              ...
              <div className="pt-4">
                <button className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-primary transition-all flex items-center justify-center gap-3 shadow-xl shadow-slate-900/10">
    ...
    <div className="max-w-3xl mx-auto px-4 py-8 md:py-20">
      <div className="space-y-6 md:space-y-8">
        <div className="flex justify-between items-center text-[10px] md:text-sm font-black text-slate-400 uppercase tracking-widest">
          ...
          <span className="text-primary">Tiến trình: {Math.round(progress)}%</span>
        </div>
          ...
          <div className="grid gap-4">
            {currentQuestion.options.map((option, idx) => (
              <button
                ...
                className="group flex items-center justify-between p-6 rounded-2xl border-2 border-slate-50 hover:border-primary hover:bg-primary-50/50 transition-all text-left relative overflow-hidden"
              >
                <div className="absolute inset-y-0 left-0 w-1 bg-primary opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <span className="text-lg font-bold text-slate-700 group-hover:text-primary-dark">{option.text}</span>
                <ChevronRight className="w-6 h-6 text-slate-200 group-hover:text-primary translate-x-0 group-hover:translate-x-2 transition-all shrink-0" />
              </button>
            ))}
          </div>
        </div>
