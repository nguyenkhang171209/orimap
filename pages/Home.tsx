  ...
  return (
    <div className="flex flex-col bg-white overflow-hidden">
      ...
      <section className="relative pt-8 pb-12 md:pt-12 md:pb-20 lg:pt-20 lg:pb-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
          ...
          <div className="flex justify-center mb-6 md:mb-10 lg:mb-16">
            <div className="inline-flex items-center gap-2 md:gap-3 bg-primary-50/50 px-4 md:px-6 py-2 md:py-2.5 rounded-full border border-primary-100 text-primary text-xs md:text-sm lg:text-lg font-bold tracking-tight animate-fade-in text-center">
          ...
          <div className="bg-secondary rounded-3xl md:rounded-[3.5rem] py-10 md:py-16 lg:py-20 px-5 md:px-8 lg:px-16 max-w-6xl mx-auto shadow-[0_40px_100px_-20px_rgba(255,140,0,0.3)] overflow-hidden relative group">
            ...
            <div className="grid lg:grid-cols-[1.3fr_0.7fr] gap-8 md:gap-12 lg:gap-16 items-center relative z-10">
              ...
              <div className="space-y-6 text-center lg:text-left text-white max-w-2xl mx-auto lg:mx-0">
                <h1 className="text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-[800] leading-[1.2] md:leading-[1.1] tracking-tighter whitespace-normal flex flex-wrap justify-center lg:justify-start items-center gap-x-2 md:gap-x-3 gap-y-1 md:gap-y-2">
                  ...
                  <span className="text-primary-dark shrink-0">Tương Lai</span>
                ...
                <div className="flex flex-col sm:flex-row gap-4 md:gap-5 justify-center lg:justify-start pt-2">
                  <button 
                    ...
                    className="w-full sm:w-auto bg-primary-dark text-white px-8 md:px-10 py-4 md:py-5 rounded-2xl md:rounded-3xl font-[800] text-base md:text-lg hover:bg-primary hover:shadow-2xl hover:shadow-primary-dark/30 transition-all flex items-center justify-center gap-3 active:scale-95 group/btn"
                  >
                    Khám phá ngay <ArrowRight className="w-5 h-5 group-hover/btn:translate-x-1 transition-transform" />
                  </button>
                  <button 
                    onClick={() => onNavigate('roadmap')}
                    className="w-full sm:w-auto bg-white text-secondary px-8 md:px-10 py-4 md:py-5 rounded-2xl md:rounded-3xl font-[800] text-base md:text-lg hover:bg-secondary-50 transition-all flex items-center justify-center gap-3 shadow-xl active:scale-95"
                  >
                ...
                <div className="flex flex-col md:flex-row items-center justify-center lg:justify-start pt-4 md:pt-2 gap-4">
                  <div className="flex -space-x-3 md:-space-x-4">
                    {[1,2,3,4].map(i => (
                      <div key={i} className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 md:border-4 border-secondary bg-white overflow-hidden shadow-lg">
                        <img src={`https://i.pravatar.cc/150?u=${i}`} className="w-full h-full object-cover" alt="Student" />
                      </div>
                    ))}
                    <div className="w-10 h-10 md:w-14 md:h-14 rounded-full border-2 md:border-4 border-secondary bg-primary-dark flex items-center justify-center text-white text-[10px] md:text-xs font-bold shadow-lg">
                      +50k
                    </div>
                  </div>
      ...
      <section className="pt-16 pb-12 md:pt-24 md:pb-20 bg-slate-50/50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-12 md:space-y-20">
          <div className="text-center space-y-4 md:space-y-6">
            <h2 className="text-2xl md:text-4xl lg:text-5xl font-[800] text-slate-900 leading-tight">Trải nghiệm hướng nghiệp <br className="md:hidden" /><span className="text-secondary">khác biệt</span></h2>
            <p className="text-muted-text max-w-2xl mx-auto text-base md:text-xl font-medium">Kết hợp dữ liệu tuyển sinh thực tế và thuật toán AI tiên tiến nhất.</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-10">
            {[
              { title: 'Phân tích đa chiều', desc: 'Sử dụng khung năng lực Holland kết hợp Big Data để đánh giá chính xác tiềm năng.', icon: BrainCircuit, color: 'var(--color-primary)' },
              { title: 'Tối ưu lộ trình', desc: 'Thiết kế từng bước đi cụ thể từ lớp 10 đến khi ra trường cho từng cá nhân.', icon: Zap, color: 'var(--color-secondary)' },
              { title: 'Dữ liệu thời gian thực', desc: 'Luôn cập nhật xu hướng thị trường, mức lương và nhu cầu tuyển dụng mới nhất.', icon: TrendingUp, color: 'var(--color-primary-dark)' }
            ].map((f, i) => (
              <div key={i} className="bg-white p-8 md:p-12 rounded-2xl md:rounded-[3rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] border border-slate-100 hover:border-slate-200 transition-all lg:hover-lift group">
                <div style={{ backgroundColor: `color-mix(in srgb, ${f.color} 10%, transparent)`, color: f.color }} className="w-14 h-14 md:w-20 md:h-20 rounded-2xl md:rounded-3xl flex items-center justify-center mb-6 md:mb-10 group-hover:scale-110 transition-transform">
                  <f.icon className="w-7 h-7 md:w-10 md:h-10" />
                </div>
                <h3 className="text-xl md:text-3xl font-[800] text-slate-900 mb-3 md:mb-5">{f.title}</h3>
                <p className="text-muted-text leading-relaxed text-sm md:text-lg font-medium">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-12 md:gap-24 items-center">
            <div className="lg:w-1/2 space-y-8 md:space-y-12">
              <h2 className="text-3xl md:text-5xl font-[800] text-primary-dark leading-tight text-center lg:text-left">Chỉ với 3 bước đơn giản</h2>
              <div className="space-y-8 md:space-y-10">
                {[
                  ...
                  <div key={i} className="flex flex-col md:flex-row items-center md:items-start gap-4 md:gap-8 group text-center md:text-left">
                    <div className="flex-shrink-0 w-14 h-14 md:w-20 md:h-20 bg-secondary text-white rounded-2xl md:rounded-[2rem] flex items-center justify-center font-[900] text-2xl md:text-3xl shadow-2xl shadow-secondary/20 lg:group-hover:rotate-6 transition-transform">
                      {s.step}
                    </div>
                    <div className="space-y-1 md:space-y-2">
                      <h3 className="text-xl md:text-3xl font-[800] text-slate-900">{s.title}</h3>
                      <p className="text-muted-text text-sm md:text-xl font-medium leading-relaxed">{s.desc}</p>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <section className="py-12 md:py-24 bg-slate-50/30">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-primary rounded-3xl md:rounded-[4rem] p-8 md:p-16 lg:p-24 text-center text-white space-y-8 md:space-y-12 relative overflow-hidden shadow-2xl shadow-primary/20">
            ...
            <div className="space-y-4 md:space-y-6">
              ...
              <p className="text-primary-100 text-base md:text-2xl max-w-3xl mx-auto font-medium opacity-90">Gia nhập cộng đồng hơn 50,000 học sinh đang chuẩn bị cho sự nghiệp bứt phá.</p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-4 md:gap-5 justify-center pt-4 md:pt-8 max-w-3xl mx-auto">
              <input 
                ...
                className="px-6 md:px-10 py-4 md:py-6 rounded-2xl md:rounded-3xl bg-white/10 backdrop-blur-md border border-white/20 text-white placeholder:text-primary-100/50 w-full focus:ring-4 ring-secondary/50 outline-none text-base md:text-xl font-semibold transition-all" 
              />
              <button className="w-full sm:w-auto bg-secondary text-white px-8 md:px-12 py-4 md:py-6 rounded-2xl md:rounded-3xl font-[900] text-base md:text-xl hover:bg-secondary-dark transition-all shadow-2xl hover:shadow-secondary/40 active:scale-95 whitespace-nowrap">
                Đăng ký ngay
              </button>
            </div>
            <p className="text-primary-100/40 text-[10px] md:text-sm font-[800] uppercase tracking-[0.3em]">Hoàn toàn miễn phí cho tất cả học sinh</p>
          </div>
        </div>
      </section>
    </div>
