      ...
      <div className="mb-10 md:mb-20 text-center">
        <div className="space-y-3 md:space-y-4 mb-8 md:mb-10">
          <h1 className="text-2xl md:text-5xl lg:text-6xl font-black text-slate-900 tracking-tighter leading-tight">
            Tìm kiếm <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-primary-light">Ngành Học</span>
          </h1>
        ...
        <div className="relative group max-w-4xl mx-auto" ref={suggestionRef}>
          <div className="absolute -inset-1 bg-gradient-to-r from-primary via-primary-light to-secondary rounded-2xl md:rounded-[2.5rem] blur opacity-15 group-focus-within:opacity-30 transition duration-1000 group-focus-within:duration-200"></div>
          ...
          <div className="relative">
            <div className="absolute inset-y-0 left-0 pl-4 md:pl-7 flex items-center pointer-events-none z-10">
              {isAiLoading ? (
                ...
                <SearchIcon className="h-5 w-5 md:h-6 md:w-6 text-primary-light group-focus-within:text-primary transition-colors" />
              )}
            </div>
            <input
              type="text"
          ...
          {showSuggestions && (searchTerm.length >= 3) && (
            <div className="absolute top-full left-0 right-0 mt-4 bg-white/90 backdrop-blur-2xl border border-slate-200 rounded-[3rem] shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-4 duration-300">
              <div className="p-6 border-b border-slate-100 bg-primary-50/30 flex items-center justify-between">
                <span className="flex items-center gap-3 text-xs font-black uppercase tracking-widest text-primary">
                  ...
                  aiSuggestions.map((sug, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className="w-full text-left p-5 rounded-[2rem] hover:bg-white hover:shadow-xl hover:shadow-primary/5 transition-all group flex items-start gap-5 border border-transparent hover:border-primary-100/50 mb-1 last:mb-0"
                      >
                        ...
                        <div className="flex-1">
                          <p className="font-black text-slate-900 text-lg leading-tight group-hover:text-primary transition-colors">{major.majorName}</p>
                          <p className="text-sm text-slate-500 font-bold mb-2">{major.university}</p>
                          <div className="flex items-center gap-2">
                             <span className="px-2 py-0.5 bg-primary-50 text-primary rounded-md text-[9px] font-black uppercase tracking-tighter">AI Insight</span>
                             <p className="text-xs text-primary-light font-bold italic opacity-80">"{sug.reason}"</p>
                          </div>
                        </div>
                        <ArrowUpRight className="w-5 h-5 text-slate-300 group-hover:text-primary-light group-hover:translate-x-1 group-hover:-translate-y-1 transition-all" />
                      </button>
                    );
                  })
                ) : !isAiLoading ? (
          ...
          <div className="bg-white p-6 md:p-8 rounded-2xl md:rounded-[3rem] border border-slate-200/60 shadow-sm lg:sticky lg:top-28 space-y-6 md:space-y-10">
            <div className="flex items-center justify-between border-b border-slate-100 pb-4 md:pb-6">
              <h3 className="font-black text-base md:text-lg flex items-center gap-2 md:gap-3">
                <SlidersHorizontal className="w-4 h-4 md:w-5 md:h-5 text-primary" /> Bộ lọc
              </h3>
              <button 
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="text-sm font-black text-primary bg-primary-50 px-2 py-0.5 rounded-lg">{maxScore.toFixed(1)}</span>
              </div>
              <input 
                type="range" 
            ...
            <div className="space-y-4">
              ...
              <div className="relative">
                <select 
                  className="w-full p-4 pl-6 rounded-2xl border border-slate-200 bg-slate-50 outline-none focus:ring-4 ring-primary-50 focus:bg-white transition-all appearance-none font-bold text-slate-700 text-sm"
              ...
              <div className="space-y-3">
                {uniTypes.map(type => (
                  <label key={type.id} className="flex items-center gap-3 cursor-pointer group">
                    ...
                    />
                    <span className={`text-sm font-bold transition-colors ${selectedTypes.includes(type.id) ? 'text-primary' : 'text-slate-600 group-hover:text-slate-900'}`}>
                      {type.label}
                    </span>
                  </label>
                ))}
        ...
        <main className="flex-1">
          {sortedFilteredMajors.length > 0 ? (
                ...
                return (
                  <div 
                    ...
                    className={`bg-white rounded-2xl md:rounded-[2.5rem] border p-5 md:p-8 shadow-sm hover:shadow-xl transition-all group flex flex-col border-b-4 ${
                      isFavorited 
                        ? 'border-primary-100 ring-2 ring-primary-50/50 border-b-primary bg-primary-50/5' 
                        : 'border-slate-100 border-b-transparent hover:border-b-primary'
                    }`}
                  >
                    <div className="flex justify-between items-start mb-4 md:mb-6">
                      <div className="space-y-2 w-full">
                        <div className="flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2">
                            {isFavorited && (
                              <span className="flex items-center gap-1 px-2 py-1 bg-primary text-white rounded-lg text-[8px] font-black uppercase tracking-widest shadow-md shadow-primary/20">
                        ...
                        <h3 className="text-lg md:text-2xl font-black text-slate-900 group-hover:text-primary transition-colors leading-tight">{major.majorName}</h3>
                    ...
                    <div className="grid grid-cols-2 gap-3 md:gap-4 mb-6 md:mb-8">
                      <div className="bg-white p-3 md:p-4 rounded-xl md:rounded-2xl border border-slate-100 group-hover:bg-primary-50/30 transition-colors">
                        <span className="text-[8px] md:text-[9px] text-slate-400 font-black uppercase tracking-widest block mb-1">Điểm chuẩn</span>
                        <p className="text-xl md:text-2xl font-black text-primary">{major.score.toFixed(1)}</p>
                      </div>
                      <div className="bg-white p-3 md:p-4 rounded-xl md:rounded-2xl border border-slate-100 group-hover:bg-primary-50/30 transition-colors">
                    ...
                    <div className="flex flex-wrap gap-2 mb-8 md:mb-10">
                      {major.blocks.map(b => (
                        <span key={b} className="px-2 md:px-3 py-1 md:py-1.5 bg-white border border-slate-200 text-slate-600 rounded-lg md:rounded-xl text-[8px] md:text-[10px] font-black tracking-widest group-hover:border-primary-100 transition-colors">
                          {b}
                        </span>
                      ))}
                    </div>
                    ...
                    <div className="mt-auto pt-4 md:pt-6 border-t border-slate-100 flex gap-2 md:gap-3">
                      <button 
                        ...
                        className="flex-1 bg-slate-900 text-white py-3 md:py-4 rounded-xl md:rounded-2xl font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-primary transition-all flex items-center justify-center gap-2 shadow-lg shadow-slate-900/10"
                      >
            ...
            <div className="flex flex-col items-center justify-center py-16 md:py-24 text-center bg-white rounded-2xl md:rounded-[3rem] border border-slate-100 border-dashed animate-in fade-in duration-500">
              <div className="w-16 h-16 md:w-24 md:h-24 bg-primary-50 rounded-full flex items-center justify-center mb-6 md:mb-8">
                <AlertCircle className="w-8 h-8 md:w-12 md:h-12 text-primary-100" />
              </div>
              ...
              <button 
                onClick={resetFilters}
                className="px-6 md:px-8 py-3 md:py-4 bg-primary text-white rounded-xl md:rounded-2xl font-black text-xs md:text-sm uppercase tracking-widest hover:bg-primary-dark transition-all shadow-xl shadow-primary/20"
              >
                Xóa tất cả bộ lọc
              </button>
            </div>
